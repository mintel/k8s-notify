from utils import ANNOTATION_PUBLIC_URL, \
...
class Receiver(object):
    ...
    def _rollout_status(self, condition,
        ...
        if num_replicas_unavailable > 0 and num_replicas_current != num_replicas_updated:
    ...
    def _handle_deployment_change(self, deployment):
        ...
        deployment_key = f"{status.observed_generation}/{metadata.namespace}/{metadata.name}"

        previous_generation = self.rollouts_previous_generation.get(deployment_key, 0)
        if previous_generation > 0 and status.observed_generation == previous_generation:
            return
        ...
        message_summary = ("Desired:%s, Ready:%s, Updated:%s, Available:%s, Unavailable:%s" % \
            (num_replicas_desired, num_replicas_ready, num_replicas_updated, num_replicas_available, num_replicas_unavailable))

        message_completion = "%d/%d (%d %%)" % (num_replicas_updated, num_replicas_desired, int(100 * num_replicas_updated/num_replicas_desired))

        rollout_status  = \
            self._rollout_status(last_condition,
                                num_replicas_desired,
                                num_replicas_updated,
                                num_replicas_ready,
                                num_replicas_current,
        ...
        if self.rollout_complete(rollout_status):
            ...
            self.rollouts_previous_generation[deployment_key] = status.observed_generation
